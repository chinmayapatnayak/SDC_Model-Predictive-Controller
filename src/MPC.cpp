 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N - 1; t++) {
      fg[0] += w_steer * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += w_acc   * CppAD::pow(vars[a_start + t], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (size_t t = 0; t < N - 2; t++) {
      fg[0] += w_del_steer * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += w_del_acc   * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }  
    ...
    for (size_t t = 0; t < N - 1; t++) {
      ...
      // Correcting for consistency with simulator : Positive --> Right Turn/Clockwise
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Return Variable
  vector<double> x1;
  x1.push_back(solution.x[delta_start]); // Return Steer
  x1.push_back(solution.x[a_start]); // Return Acceleration

  // Return predicted points for a state for visualization
